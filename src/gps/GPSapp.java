	...
	private void getMapSelector(Building building, Pane root, ImageView imageView) {
    	...
    	for(int i = 1; i <= building.getNumMaps(); i++){
    		...
    		mapImageView.setImage(image);
   	     	...
   	     	//TEST GARABAGE COLLECTION, call the deletion method,
   	     	//have it spin in that method until global variable 
   	     	//timeToCollect is set to true, then actually delete them
   	     	//deleteGarabe(image, mapImageView, g1);
   	     	
    	}
	}
    
    //DOESNT WORK .. UNLESS WE USE THREADS
    //SWITCH TO A TIMED GARBAGE COLLECTOR, SO IT WILL JUST CLEAR OUR THINGS
    //AFTER ABOUT A MINUTE, AND THEN CHECK TO SEE IF WE HAVE NOT NAVIGATED AWAY
    //FROM THIS SCREEN, IF WE HAVENT, RELOAD THIS SAME SCREEN AGAIN
    //WE PROBABLY WONT BE ON THIS SCREEN THAT LONG SO IT WOULD BE OKAY TO DELETE THESE THINGS
    //AFTER X AMOUNT OF TIME..
    private void deleteGarabe(Image image, ImageView mapImageView, Group g1) {
		while(!timeToCollect){
			//spin until we want to delete these objects
		}
		image = null;
		mapImageView = null;
		g1 = null;
